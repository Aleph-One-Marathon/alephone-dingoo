...
inline _fixed Float2Fixed(float FValue)
{
	return (FValue >= 0) ? _fixed(FIXED_ONE*FValue + 0.5) : - _fixed(FIXED_ONE*(-FValue) + 0.5);
}

bool XML_ShellCasingParser::HandleAttribute(const char *Tag, const char *Value)
{
	float FValue;
...
bool XML_ShellCasingParser::AttributesDone()
{
	// Verify...
	if (!IndexPresent)
	{
	...
	shell_casing_definition& OrigData = shell_casing_definitions[Index];
	
	if (IsPresent[0]) OrigData.collection = Data.collection;
	if (IsPresent[1]) OrigData.shape = Data.shape;
	if (IsPresent[2]) OrigData.x0 = Data.x0;
	if (IsPresent[3]) OrigData.y0 = Data.y0;
	if (IsPresent[4]) OrigData.vx0 = Data.vx0;
	if (IsPresent[5]) OrigData.vy0 = Data.vy0;
	if (IsPresent[6]) OrigData.dvx = Data.dvx;
	if (IsPresent[7]) OrigData.dvy = Data.dvy;
...
bool XML_WeaponOrderParser::AttributesDone()
{
	// Verify...
	if (!IndexPresent || !WeaponPresent)
	{
		AttribsMissing();
		return false;
	}
	
	weapon_ordering_array[Index] = Weapon;
...
static XML_ElementParser WeaponsParser("weapons");


// XML-parser support
XML_ElementParser *Weapons_GetParser()
{
	WeaponsParser.AddChild(&ShellCasingParser);
